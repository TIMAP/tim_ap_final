...
public class AdminController {
			...
			// }
				...
				if (member.getPw().equals(admin.getPw())) {
					if (admin.getRole() == 'Y') {
						...
						String name = admin.getName_first()
								+ admin.getName_last();
					...
					boolean msg = false;
					result.addObject("msg", "비밀번호가 일치하지 않습니다.");
					result.addObject("failed", msg);
					result.setViewName("/admin/adminlogin");
				}
	...
	public ModelAndView main(HttpSession session,
		...
		int id = 0;
		if (session.getAttribute("id") != null) { // 세션에 아이디가 있을 경우 id변수에 세션에 담긴
													// id를 담아준다.
			id = (int) session.getAttribute("id");
		}
		if (id == 0) {// 세션에 id가 담기지 않은 경우 id변수는 0이기 때문에, 로그인한 상태가 아니므로 로그인폼으로
						// 보낸다 그리고 msg를 가지고 가서 alert를 띄어준다.
			boolean msg = false;
			...
			result.setViewName("admin/adminPage");// 로그인한 상태에서 main 즉 홈화면으로 가게
													// 될경우 바로 보내주는 것.
		}
		return result;
	}
	...
	public ModelAndView list(Locale locale,
		...
		List<MemberEntity> memberList = memberService
				.getMemberList(pagingEntity);
		pagingEntity.setDataList(memberList);
		pagingEntity.setTotalRecords(memberService
				.selectMemberListTotalCount(pagingEntity));
	...
	public ModelAndView joinForm() {
		ModelAndView result = new ModelAndView();
		result.setViewName("/admin/addMembers");
		return result;
	}
