...
<div style="text-align: center; padding-top: 5%" class="mainDiv">
	<div>
		<form action="/conference/conferencelist">
			...
			<input type="button" class="btn btn-default loginButton joinButton conferenceSelect" value="회의 추가" onclick="location.href='/audio/form' " class="btn">
			<input type="button" class="btn btn-default loginButton joinButton conferenceSelect" value="뒤로 가기" onclick="history.back(-1);" class="btn">
		</form>
	</div>
	<table class="table table-bordered table-hover conferenceList" style="margin: 1% auto 1% auto;">
		<tr>
			<th>No.</th>
			<th>제목</th>
			<th>등록일</th>
	...
	<%-- 		<c:when test="${viewData.boardTotalCount > 0 }"> --%>
 			<c:forEach items="${viewData.conferList }" var="confer" varStatus="i">
           			<tr>
              				 <td><a href="/audio/list?c_id=${confer.id }">${confer.id }</a></td>
             				 <td>${confer.title }</td>
               				<td>${confer.date}</td>
               				<td>${confer.role }</td>
		             		  <td>${confer.entry }</td>
